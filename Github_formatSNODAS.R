...
#read in shapefile - change to where your shapefile is stored
shapefile_area<-readOGR("/Users/keirajohnson/Box Sync/Keira_Johnson/HJAshapefile/layers/globalwatershed.shp")
...
#save matrix as RData object  
save(swe_matrix_mm, "swe_matrix_mm.RData")

dateList_rmNULL<-dateList[-which(sapply(dateList, is.null))]

swe_rmNULL<-swe_matrix_mm[-which(sapply(swe_matrix_mm, is.null))]

number_rows<-nrow(swe_matrix_mm[[1]])
number_cols<-ncol(swe_matrix_mm[[1]])

#turn into array (3D matrix - [lat, long, date])
swe_array<-array(as.numeric(unlist(swe_matrix_mm)), dim=c(number_rows, number_cols, length(swe_matrix_mm)))

write.csv(swe_array, "SWE_Array.csv")
